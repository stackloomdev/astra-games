...
// A section that mixes experiments with multiplayer games cannot be classified
// from its heading, so the entry's own description decides.
const SECTION_MIXED = /实验玩法|実験的な作品|실험적 게임|Экспериментальные и многопользовательские|التجريبية ومتعددة|प्रयोगात्मक और मल्टीप्लेयर|experimental.*multiplayer|Exp[ée]rimental et multijoueur|Experimentelles und Mehrspieler|Experimentales y multijugador|Experimentais e multijogador|Eksperimental dan multipemain/i;
...
export function classifyWork(sourceCategory, description = '', parents = '') {
  ...
  if (SECTION_MIXED.test(sourceCategory)) return classify(description) || 'experiment';
