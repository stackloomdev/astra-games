// Ported verbatim from MartinDelophy/awesome-gpt-6-astra `website/server/catalog.js`.
// Parses the upstream README into the work catalogue so this site never keeps a
// second copy of the list. Only the fallback-snapshot loader was changed, to a
// static import that survives Next.js bundling.
import { createHash } from 'node:crypto';
import fallbackEn from './catalog-fallback.en.json' with { type: 'json' };
import fallbackZhCN from './catalog-fallback.zh-CN.json' with { type: 'json' };
...
// Upstream keeps one README per locale. This site's chrome is Simplified
// Chinese, so it reads the matching list; every locale file sits at the repo
// root, so relative link resolution is unaffected by the choice.
export const README_PATH = process.env.ASTRA_README_PATH || 'README.md';
export const readmeUrlFor = (path) => `https://github.com/${REPOSITORY}/blob/main/${path}`;
export const rawUrlFor = (path) => `https://raw.githubusercontent.com/${REPOSITORY}/main/${path}`;
export const README_URL = readmeUrlFor(README_PATH);
export const RAW_URL = rawUrlFor(README_PATH);
...
// Upstream maintains the same list in twelve languages, and each translates the
// bullet labels. These alternations carry every label actually in use, so author
// names, source links and preview rows are read identically in every language
// rather than only in the two the parser originally knew.
const L_AUTHOR = '作者|创作者|제작자|Автор|المبدع|रचनाकार|Kreator|Créateur|Entwickler|Creador|Criador|author|creator|by';
const L_PLATFORM = '平台|対応環境|플랫폼|Платформа|المنصة|प्लैटफ़ॉर्म|Plateforme|Plattform|Plataforma|platform';
const L_MODEL = 'GPT[ -]?6(?:\\s+Astra)?|模型参与|モデルの担当範囲|모델 참여|Участие модели|مشاركة النموذج|मॉडल की भागीदारी|Keterlibatan model|Participation du modèle|Modellbeteiligung|Participación del modelo|Participação do modelo';
const L_SOURCE = '开发资料|開発資料|源码|源代码|仓库|개발 자료|Материалы|موارد التطوير|विकास संसाधन|Materi pengembangan|Ressources|Materialien|Recursos|resources?|source(?: code)?|repository|repo';
const L_PREVIEW = '预览|截图|プレビュー|미리보기|Скриншот|معاينة|झलक|Pratinjau|Aperçu|Vorschau|Vista previa|Prévia|preview|screenshot';
const L_DEMO = '体验|试玩|演示|在线|demo|live|play|website';
const L_TECH = '技术|technology|built with';

const META_LABEL = new RegExp(`^(?:${L_AUTHOR}|${L_PLATFORM}|${L_MODEL}|${L_SOURCE}|${L_PREVIEW}|${L_DEMO}|${L_TECH})\\s*[:：]`, 'i');
const AUTHOR_LABEL = new RegExp(`^(?:${L_AUTHOR})\\s*[:：]`, 'i');
const SOURCE_LABEL = new RegExp(`^(?:${L_SOURCE})\\s*[:：]`, 'i');
const DEMO_LABEL = new RegExp(`^(?:${L_DEMO})\\s*[:：]`, 'i');
const PREVIEW_LABEL = new RegExp(`^(?:${L_PREVIEW})$`, 'i');
// Within a resources bullet the source link is usually labelled; when it is not,
// callers fall back to the bullet's first link.
const SOURCE_LINK = /源码|源代码|ソース|소스|Исходник|المصدر|सोर्स|source|quellcode|código|codice|kode|repository|\brepo\b/i;
...
// Category detection reads the upstream section heading, which is translated in
// every language. Without the non-Latin and non-English terms below, ten of the
// twelve catalogues classified every entry as "other".
//
// Latin and Cyrillic terms are bounded so short ones ("art", "jeu", "игр") do
// not match inside longer words. CJK, Arabic and Devanagari have no word
// boundaries for \b to find, so those terms match as plain substrings.
const CAT_WEBSITE = /网站|网页|落地页|官网|ウェブサイト|웹사이트|сайт|موقع|वेबसाइट|\b(?:websites?|landing pages?|web design|sites? web|webseiten?|sitios? web|situs)\b/i;
const CAT_APP = /应用|移动端|桌面端|アプリ|앱|모바일|приложени|تطبيق|ऐप|\b(?:apps?|applications?|mobile|desktop|anwendungen?|aplicaci[oó]n(?:es)?|aplicativos?|aplikasi)\b/i;
const CAT_TOOL = /工具|效率|自动化|ツール|도구|инструмент|أدوات|उपकरण|\b(?:tools?|utilities|productivity|automation|outils?|werkzeuge?|herramientas?|ferramentas?|alat)\b/i;
const CAT_EXPERIMENT = /艺术|实验|可视化|粒子|创意编程|実験|アート|可視化|パーティクル|실험|아트|예술|시각화|파티클|эксперимент|искусств|визуализац|частиц|تجريب|فن|جسيمات|प्रयोग|कला|विज़ुअलाइज़|कण|\b(?:experiments?|experimental|creative coding|visuali[sz]ations?|art|exp[ée]rimenta\w*|visualisations?|particules|experimentelles|kunst|visualisierung|partikel|experimentales|arte|visualizaci[oó]n|part[ií]culas|experimentais|visualiza[çc][ãa]o|eksperimental|seni|visualisasi|partikel)\b/i;
const CAT_GAME = /游戏|动作|街机|解谜|益智|策略|模拟|冒险|跳跃|竞速|ゲーム|アクション|アーケード|パズル|頭脳|戦略|シミュレーション|プラットフォーム|レース|冒険|게임|액션|아케이드|퍼즐|두뇌|전략|시뮬레이션|플랫포머|레이싱|어드벤처|игр|экшен|аркад|головоломк|логическ|стратеги|симулятор|платформер|гонк|لعب|ألعاب|الحركة|الأركيد|الألغاز|الاستراتيجي|المحاكاة|المنصات|السباقات|खेल|एक्शन|आर्केड|पहेलियाँ|दिमागी|रणनीति|सिमुलेशन|प्लैटफ़ॉर्मर|रेसिंग|\b(?:games?|arcade|puzzles?|strategy|simulation|rpg|adventure|racing|platformers?|action|jeux?|casse-t[êe]tes?|r[ée]flexion|strat[ée]gie|plateforme|course|spiele?|r[äa]tsel|denkspiele|strategie|plattform|rennspiele|juegos?|acci[oó]n|puzles|ingenio|estrategia|simulaci[oó]n|plataformas?|carreras|jogos?|a[çc][ãa]o|quebra-cabe[çc]as|racioc[ií]nio|estrat[ée]gia|simula[çc][ãa]o|corrida|permainan|aksi|arkade|teka-teki|asah otak|strategi|simulasi|balapan)\b/i;

/**
 * Coarse type, for structured data and the JSON API. Read only from the headings
 * an entry sits under, never from its description: descriptions are translated,
 * and keyword-matching them put the same work under a game in one language and
 * a website in another. Upstream's headings line up across all twelve READMEs,
 * so this does too. The section heading decides; the headings above it are the
 * fallback for a section whose title names no type, such as "RPGs & adventures"
 * under "Games".
 */
export function classifyWork(sourceCategory, parents = '') {
  const classify = (text) => {
    if (CAT_WEBSITE.test(text)) return 'website';
    if (CAT_APP.test(text)) return 'app';
    if (CAT_TOOL.test(text)) return 'tool';
    if (CAT_EXPERIMENT.test(text)) return 'experiment';
    if (CAT_GAME.test(text)) return 'game';
    return null;
  };
  return classify(sourceCategory) || classify(parents) || 'other';
}
...
export function parseCatalogMarkdown(markdown, options = {}) {
  ...
  const urlOptions = { repository: REPOSITORY, branch: 'main', readmePath: README_PATH, ...options };
  ...
  function addWork({ name, description, titleLink, metadata, category, authorCell, sourceCell, demoCell, imageCell }) {
    ...
    for (const paragraph of metadata || []) {
      ...
      if (DEMO_LABEL.test(text)) demoUrl ||= paragraphLinks.find(link => !isRepositoryUrl(link.url))?.url || null;
      imageUrl ||= images(paragraph)[0] || null;
    }
    if (isRepositoryUrl(titleLink)) sourceUrl ||= titleLink;
    else demoUrl ||= titleLink;
    // The nested bullets carry platform, model involvement and build notes.
    // metadata[0] is the entry's own line, already read as name + description.
    const details = [];
    for (const paragraph of (metadata || []).slice(1)) {
      const raw = plain(paragraph).trim();
      if (!raw) continue;
      const parts = raw.match(/^([^:\uff1a]{1,40})[:\uff1a]\s*([\s\S]*)$/);
      const label = parts ? parts[1].trim() : '';
      const text = (parts ? parts[2] : raw).trim();
      // A preview bullet is just the screenshot; its alt text is not a fact
      // about the work, and the image itself is already read as imageUrl.
      if (!text || PREVIEW_LABEL.test(label)) continue;
      details.push({ label, text, links: links(paragraph).map(link => ({ text: link.text, url: link.url })) });
    }
    ...
    const work = {
      ...
      category: classifyWork(sourceCategory, headings.map(h => h.text).join(' ')),
      sourceCategory,
      author,
      demoUrl,
...
// Cold-start snapshots, keyed by the list they were captured from. A snapshot is
// a copy of one language's catalogue, so it can only ever stand in for that
// language — showing the English list on the Russian page would be worse than
// showing none. Languages without a snapshot degrade to an empty catalogue with
// a retry, which the page renders as such.
const FALLBACKS = {
  'README.md': fallbackEn,
  'README.zh-CN.md': fallbackZhCN,
};

function readFallback(readmePath) {
  const snapshot = FALLBACKS[readmePath];
...
export function createCatalogService({
  // One service per language: each keeps its own cache, ETag and last-good
  // result, so a failure in one locale never affects another.
  readmePath = README_PATH,
  fetchImpl = globalThis.fetch,
  now = Date.now,
  fallback = readFallback(readmePath),
  cacheTtlMs = REFRESH_SECONDS * 1000,
  retryDelayMs = 30_000,
  timeoutMs = 8_000,
} = {}) {
  const rawUrl = rawUrlFor(readmePath);
  const readmeUrl = readmeUrlFor(readmePath);
  let lastGood = null;
  ...
  async function refresh() {
    ...
    try {
      ...
      lastGood = {
        ...
        source: {
          repository: REPOSITORY, url: readmeUrl, readmePath, checkedAt, lastSuccessfulAt: checkedAt,
      ...
      cachedResult = {
        ...
        source: {
          repository: REPOSITORY, url: readmeUrl, readmePath,
          ...(previous?.source || {}),
