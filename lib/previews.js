...
//
// Unchanged from upstream except: the Node (req, res) handler is replaced by a
// Web Response one, and the screenshot match and repository lookup moved to
// preview-version.js, where the build step's cover key shares them.
import { lookup } from 'node:dns/promises';
import { request } from 'node:https';
import { isIP } from 'node:net';
import { createHash } from 'node:crypto';
import { githubRepositoryUrl, previewVersion, screenshotFor } from './preview-version.js';
...
// Lives in preview-version.js now, so the build step's cover key reads the same
// logic as this resolver. Re-exported to keep this module's interface.
export { githubRepositoryUrl };

async function buildPreview(work, fetcher) {
  const matched = screenshotFor(work);
