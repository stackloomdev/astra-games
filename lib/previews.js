...
//
// Unchanged from upstream except: the manifest is a static import, the Node
// (req, res) handler is replaced by a Web Response one, and that handler can
// downscale on the way out (see preview-resize.js).
...
import { parsePreviewWidth } from './preview-sizes';
import { readLocalPreview, renderVariant } from './preview-resize.js';
 ...
 * current catalogue entry, nothing unexpected — and adds two: `l` for the
 * language, because a work's id is derived from its translated title, and `w`
 * for the width it will be drawn at.
 */
export function createPreviewResponder({ loadCatalogFor, isLocale, defaultLocale, getPreview = resolvePreview }) {
  ...
  return async function respond(request) {
    ...
    const width = parsePreviewWidth(params.get('w'));
    if (params.getAll('id').length !== 1 || !id || id.length > 200 || params.getAll('v').length > 1 ||
        ...
        params.getAll('w').length > 1 || (params.get('w') !== null && width === null) ||
        [...params.keys()].some(key => !['id', 'v', 'l', 'w'].includes(key))) {
    ...
    try {
      ...
      const headers = {
        ...
        // Set for every answer to a width request, including the ones that
        // decline to resize: a shared cache must not store one client's format
        // under a key that another client's Accept would also match.
        ...(width ? { Vary: 'Accept' } : null),
      };

      if (result.unavailable) return new Response(null, { status: 404, headers });

      // A width asks for the cover at the size it is drawn. Any step of this can
      // decline — no sharp, a screenshot that is not in the bundle, an encode
      // that came out bigger — and then the original is served instead.
      if (width) {
        const source = result.location
          ? await readLocalPreview(result.location)
          : { body: result.body, contentType: result.contentType };
        const variant = source && await renderVariant({
          key: `${work.id}:${previewVersion(work)}`,
          ...source,
          width,
          accept: request.headers.get('accept') || '',
        });
        if (variant) {
          return new Response(method === 'HEAD' ? null : variant.body, {
            status: 200,
            headers: {
              ...headers,
              'Content-Type': variant.contentType,
              'Content-Length': String(variant.body.length),
              'X-Preview-Width': String(variant.width),
            },
          });
        }
      }
